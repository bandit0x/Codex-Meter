...
- [ ] 当前来源版本可生成明确可定位的便携产物，并通过发布目录启动烟雾测试。
- [ ] 目标机器真实 Codex 冒烟检查对照五小时、一周、重置和新鲜度字段，且不消费 Full Reset Credit。
- [ ] 隔离数据目录端到端检查覆盖首次启动、重新打开、偏好恢复与至少一个可行动失败路径。
- [ ] 真实运行截图覆盖紧凑、加载、健康、失败和收起状态，并与批准的方案 3 逐项对照。
